/**
 * @file reaction_manager.cpp
 * @brief Implementation of class ReactionManager
 * 
 * This file contains the implementation of the ReactionManager class, which manages the chemical
 * reactions in the simulation. The class is responsible for reading reaction data, generating
 * reaction lists based on different conditions, and performing various calculations related to
 * reactions.
 * 
 * @date 2025-02-12
 * @author Y. Kawasaki
...
{
    /**
     * @brief Constructor for ReactionManager.
     * 
     * Initializes the ReactionManager with the SpeciesManager and input configuration. Reads the necessary
     * reaction files, generates the reaction lists based on various conditions (e.g., dust surface reactions, 
     * chemical desorption), and finalizes the reaction setup.
     * 
     * @param ptr_species_manager A pointer to the SpeciesManager, which manages the species in the simulation.
     ...
     * 
     * This method takes a list of species names and uses the SpeciesManager to find the corresponding
     * indices of the species in the species list. If a species is not found, it returns the value 
     * defined by `FindSpeciesIndex` for a non-existent species.
     * 
     ...
     */
    std::vector<std::size_t> ReactionManager::GetSpeciesListIndices(const std::vector<std::string>& species_name_list) 
    {
        // Initialize the result vector with the same size as the input species list
        std::vector<std::size_t> indices(species_name_list.size());

        // Use std::transform to map each species name to its corresponding index
        std::transform(
            species_name_list.begin(), 
            species_name_list.end(), 
            indices.begin(),
            [&](const std::string& species) {
                // Find the index for the species using the SpeciesManager
                return ptr_species_manager_->FindSpeciesIndex(species);
            }
        );
     ...
     * 
     * This method takes a list of species indices and uses the SpeciesManager to get the corresponding
     * species names. If an index corresponds to a non-existent species, it returns an empty string (or
     * another placeholder as defined).
     * 
     ...
     */
    std::vector<std::string> ReactionManager::GetSpeciesNameList(const std::vector<std::size_t>& indices)
    {
        // Initialize the result vector with the same size as the input indices list
        std::vector<std::string> species_name_list(indices.size());

        // Use std::transform to map each index to its corresponding species name
        std::transform(
            indices.begin(), 
            indices.end(), 
            species_name_list.begin(),
            [&](std::size_t index) { 
                // Return the species name if found, otherwise return an empty string
     ...
     * 
     * This method checks if all the species in the given reactant and product lists are present in either
     * the species name list managed by the SpeciesManager or in a special list of species (SPECIAL_SPECIES_LIST).
     * If any species is missing, the function returns false. Otherwise, it returns true.
     * 
     ...
     */
    bool ReactionManager::AreReactionSpeciesInSpeciesNameList(const std::vector<std::string>& reactants, const std::vector<std::string>& products) 
    {       
     ...
     * 
     * This method reads a gas-phase reaction file, parses each line to extract relevant data (such as reactants,
     * products, rate parameters, and temperature limits), and creates Reaction objects for valid reactions.
     * The reactions are categorized by their reaction type and stored in the internal reaction list.
     * 
     * @param filename The name of the file containing the gas-phase reaction data.
     */
    void ReactionManager::ReadGasPhaseReactionFile(std::string filename) 
    {
        ...
        while (std::getline(file, line)) {
            ...
            std::vector<std::size_t> reactant_indices = GetSpeciesListIndices(reactants);
            std::vector<std::size_t> product_indices  = GetSpeciesListIndices(products);

            // Extract reaction rate parameters from the line
            const double alpha = std::stod(split_result[8]);
            const double beta  = std::stod(split_result[9]);
            const double gamma = std::stod(split_result[10]);
            ...
            const double temperature_lower_limit = std::stod(split_result[15]);
            const double temperature_upper_limit = std::stod(split_result[16]);
            ...
            std::vector<double> rate_parameters(gas_phase_reaction_params::kNumParams);
     ...
     * 
     * This method takes a single line from a gas-phase reaction file and splits it into its constituent 
     * components, such as reactants, products, rate parameters, and other relevant information. 
     * The substrings are trimmed to remove any leading or trailing spaces, and each value is returned 
     ...
     */
    std::vector<std::string> ReactionManager::SplitGasReactionLine(const std::string& line) 
    {
    ...
    /**
     * @brief Reads and processes the dust surface reaction file.
     * 
     * This method reads a dust surface reaction file line by line, processes each reaction, 
     * and stores the relevant information (reactants, products, rate parameters, etc.) in the 
     * reaction list. It also handles special cases like chemical desorption and H2 desorption.
     *
     * @param filename The name of the file containing the dust surface reaction data.
     */
    void ReactionManager::ReadDustSurfaceReactionFile(std::string filename) 
    {
        // Open file
        std::ifstream file(filename);
        if (!file.is_open()) {
        ...
        while (std::getline(file, line)) {
            ...
            std::vector<double> rate_parameters(dust_surface_reaction_params::kNumParams);
            ...
            if (is_chemical_desorption_) {
                ...
                if (products[1] == "" && products[2] == "") {
                    ...
                    product_indices = GetSpeciesListIndices(products_cd);
                    reaction_list_.emplace_back(std::make_shared<Reaction>(
                        ++index,
                        reactant_indices,
                        product_indices,
     ...
     *
     * This function processes a line from a dust surface reaction file and splits it into the reactants,
     * products, and rate parameters (branching ratio and uncertainty on the branching ratio).
     * Each component is trimmed and returned as a vector of strings.
     *
     ...
     */
    std::vector<std::string> ReactionManager::SplitDustSurfaceReactionLine(const std::string& line) 
    {
     ...
     *
     * This function processes the surface activation energy file and updates the activation energy and tunneling effect
     * probability for both dust surface and dust mantle reactions. It also ensures that each reaction is only added once,
     * avoiding duplicates, and checks that the reactants and products are valid species.
     *
     * @param filename The name of the file containing the activation energy data for surface reactions.
     */
    void ReactionManager::ReadSurfaceActivationEnergyFile(std::string filename) 
    {
        // open file
        std::ifstream file(filename);
        if (!file.is_open()) {
        ...
        while (std::getline(file, line)) {
            ...
            const double activation_energy = std::stod(split_result[8]);
            ...
            for (auto& reaction : reaction_list_) {
                ...
                if (reactant_indices == reaction->reactant_indices_ &&
                    product_indices  == reaction->product_indices_) {

                    // Update mass and rate parameters for the reaction
                    const double mass1 = ptr_species_manager_->species_list_[reactant_indices[0]]->GetMass();
                    const double mass2 = ptr_species_manager_->species_list_[reactant_indices[1]]->GetMass();
     ...
     *
     * This function calculates the probability of tunneling effect for a reaction based on the reduced mass
     * of the reactants and the activation energy. The formula used is based on the WKB approximation.
     *
     ...
     */
    double ReactionManager::CalculateTunnellingEffectProbabilty(const double mass1, const double mass2, const double activation_energy) 
    {
     ...
     *
     * This function parses a line from the surface activation energy file into separate components:
     * reactants, products, and rate parameters. It trims and returns these components in a vector.
     *
     ...
     */
    std::vector<std::string> ReactionManager::SplitSurfaceActivationEnergyLine(const std::string& line) 
    {
        std::string reactant1 = string_utils::Trim(line.substr( 0, 11));
        std::string reactant2 = string_utils::Trim(line.substr(11, 11));
        std::string reactant3 = string_utils::Trim(line.substr(22, 11));
    ...
    void ReactionManager::GenerateReactionListForDustAndChargedParticleCollision() 
    {
        ...
        for (auto const& gas_species : ptr_species_manager_->gas_species_list_) {
            ...
            const std::size_t gas_species_index = ptr_species_manager_->FindSpeciesIndex(gas_species_name);
            ...
            if (gas_species_name == kElectron) {
                ...
                for (auto const& dust_species : ptr_species_manager_->dust_species_list_) {
                    ...
                    std::vector<double> rate_parameters(dust_and_charged_particle_collison_params::kNumParams);
            ...
            if (string_utils::IsInStringVector(ptr_species_manager_->gas_species_name_list_, neutral_gas_species_name)) {
                ...
                if (gas_species->GetCharge() > 0) {
                    ...
                    for (std::size_t ireac = 0; ireac < total_number_of_gas_phase_reactions_; ++ireac) {
                        ...
                        if ((reaction->reactant_indices_[0] == gas_species_index && reaction->reactant_indices_[1] == ptr_species_manager_->index_electron_) ||
                            (reaction->reactant_indices_[1] == gas_species_index && reaction->reactant_indices_[0] == ptr_species_manager_->index_electron_)) {
                            skip = false;
                        }
                        ...
                        for (const auto& dust_species : ptr_species_manager_->dust_species_list_) {
                            ...
                            std::size_t number_of_reactant = reaction->reactant_indices_.size();
                            std::vector<std::size_t> reactant_indices(number_of_reactant, kNotFoundSpecies);
                            // Replace the electron in the reactants with the dust species
                            for (std::size_t index = 0; index < number_of_reactant; ++index) {
                                if (reaction->reactant_indices_[index] == ptr_species_manager_->index_electron_) {
                                    reactant_indices[index] = ptr_species_manager_->FindSpeciesIndex(dust_species->GetName());
                                } else {
                                    reactant_indices[index] = reaction->reactant_indices_[index];
                                }
                            }

                            // Get the indices of the products
                            std::size_t number_of_product = reaction->product_indices_.size();
                            std::vector<std::size_t> product_indices(number_of_product, kNotFoundSpecies);
                            // Add the dust species after the collision
                            for (std::size_t index = 0; index < number_of_product; ++index) {
                                if (reaction->product_indices_[index] == kNotFoundSpecies) {
                                    product_indices[index] = ptr_species_manager_->FindSpeciesIndex(dust_species_p1->GetName());
                                    break;
                                } else {
                                    product_indices[index] = reaction->product_indices_[index];
                                }
                            }
            ...
            for (int dust_charge1 = 1; dust_charge1 <= max_dust_charge_number; ++dust_charge1) {
                ...
                for (std::size_t bin_number2 = 1; bin_number2 <= number_of_dust_bins; ++bin_number2) { // // Collision dust 2
                    ...
                    for (int dust_charge2 = -1; dust_charge2 >= min_dust_charge_number; --dust_charge2) { // negativeダストでloop
                        ...
                        std::vector<double> rate_parameters(dust_collision_params::kNumParams);
                        ...
                        rate_parameters[dust_collision_params::kDustCharge1] = static_cast<double>(reactant_dust_species1->GetCharge()) * constants::kChargeUnit;
                        rate_parameters[dust_collision_params::kDustCharge2] = static_cast<double>(reactant_dust_species2->GetCharge()) * constants::kChargeUnit;
    ...
    void ReactionManager::GenerateReactionListForNeutralSpeciesAccretionOnDustSurfaces() 
    {
        ...
        for (const auto& dust_surface_species : ptr_species_manager_->dust_surface_species_list_) {
            ...
            // Reaction parameters
            std::vector<double> rate_parameters(accretion_on_dusts_params::kNumParams); 
    ...
    void ReactionManager::GenerateReactionListForThermalDesorptionOnDustSurfaces() 
    {
        ...
        for (const auto& dust_surface_species : ptr_species_manager_->dust_surface_species_list_) {
            ...
            // Reaction parameters
            std::vector<double> rate_parameters(thermal_desorption_params::kNumParams);
    ...
    void ReactionManager::GenerateReactionListForCosmicRayDesorptionOnDustSurfaces() 
    {
        ...
        for (const auto& dust_surface_species : ptr_species_manager_->dust_surface_species_list_) {
            ...
            // Reaction parameters
            std::vector<double> rate_parameters(cosmic_ray_desorption_on_dusts_surfaces_params::kNumPramas);
    ...
    void ReactionManager::GenerateReactionListForPhotoDesorptionByExternalUV() 
    {
        ...
        for (const auto& dust_surface_species : ptr_species_manager_->dust_surface_species_list_) {
            ...
            // Reaction parameters
            std::vector<double> rate_parameters(photo_desorption_by_external_UV_params::kNumParams);
    ...
    void ReactionManager::GenerateReactionListForPhotoDesorptionByCosmicRayGeneratedUV() 
    {
        ...
        for (const auto& dust_surface_species : ptr_species_manager_->dust_surface_species_list_) {
            ...
            // Reaction parameters
            std::vector<double> rate_parameters(photo_desorption_by_CR_generated_UV_params::kNumParams);
    ...
    void ReactionManager::GenerateReactionListForPhotoDissociationInducedByCRsOnDustSurfaces() 
    {
        ...
        for (const auto& reaction : reaction_list_) {
            ...
            std::vector<std::string> reactants = GetSpeciesNameList(reaction->reactant_indices_);
            ...
            std::vector<std::string> products = GetSpeciesNameList(reaction->product_indices_);
            ...
            std::vector<double> rate_parameters(photo_dissociation_by_CR_on_dusts_params::kNumParams);
    ...
    void ReactionManager::GenerateReactionListForPhotoDissociationByExternalUVOnDustSurfaces() 
    {
        ...
        for (const auto& reaction : reaction_list_) {
            ...
            // Set up rate parameters for the reaction
            std::vector<double> rate_parameters(photo_dissociation_by_UV_on_dusts_params::kNumParams);
    ...
    void ReactionManager::GenerateReactionListForDustMantleReaction() 
    {
        ...
        for (const auto& reaction : reaction_list_) {
            ...
            std::vector<double> rate_parameters(dust_mantle_reaction_params::kNumParams);
    ...
    void ReactionManager::GenerateReactionListForDustSurfaceToMantleSwapping() 
    {
        ...
        for (const auto& dust_surface_species : ptr_species_manager_->dust_surface_species_list_) {
            ...
            std::vector<double> rate_parameters(dust_surface_to_mantle_params::kNumPramas);
    ...
    void ReactionManager::GenerateReactionListForDustMantleToSurfaceSwapping() 
    {
        ...
        for (const auto& dust_mantle_species : ptr_species_manager_->dust_mantle_species_list_) {
            ...
            std::vector<double> rate_parameters(dust_mantle_to_surface_params::kNumPramas);
    ...
    void ReactionManager::CalculateReactionBranchingRatio() 
    {
        ...
        for (std::size_t ireac = ireac_start; ireac <= ireac_end; ++ireac) {
            ...
            std::size_t idx_ith_r1 = ith_reaction->reactant_indices_[0];
            std::size_t idx_ith_r2 = ith_reaction->reactant_indices_[1];
            ...
            for (std::size_t jreac = ireac_start; jreac <= ireac_end; ++jreac) {
                ...
                std::size_t idx_jth_r1 = jth_reaction->reactant_indices_[0];
                std::size_t idx_jth_r2 = jth_reaction->reactant_indices_[1];
            ...
            if (number_of_branch > 0) {
                ith_reaction->branching_ratio_ = 1.0 / static_cast<double>(number_of_branch);
            } else {
        ...
        for (const auto& ith_reaction : reaction_list_) {
            ...
            for (const auto& jth_reaction : reaction_list_) {
                ...
                if ((idx_ith_r1 == idx_jth_r1 && idx_ith_r2 == idx_jth_r2) || 
                    ...
                    std::size_t idx_jth_p1 = jth_reaction->product_indices_[0];
                    std::size_t idx_jth_p2 = jth_reaction->product_indices_[1];
                    std::size_t idx_jth_p3 = jth_reaction->product_indices_[2];
    ...
    void ReactionManager::CalculateChemicalDesorptionProbabilities()
    {
        for (auto& reaction : reaction_list_) {
            ...
            const double enthalpy_r1 = ptr_species_manager_->GetSpeciesEnthalpyOfFormation(idx_r1);
            const double enthalpy_r2 = ptr_species_manager_->GetSpeciesEnthalpyOfFormation(idx_r2);
            const double enthalpy_p1 = ptr_species_manager_->GetSpeciesEnthalpyOfFormation(idx_p1);
            const double enthalpy_p2 = (idx_p2 != kNotFoundSpecies ? ptr_species_manager_->GetSpeciesEnthalpyOfFormation(idx_p2) : 0.0);
            const double enthalpy_p3 = (idx_p3 != kNotFoundSpecies ? ptr_species_manager_->GetSpeciesEnthalpyOfFormation(idx_p3) : 0.0);

            // Calculate the reaction enthalpy (kJ/mol)
            const double reaction_enthalpy = (enthalpy_p1 + enthalpy_p2 + enthalpy_p3) - (enthalpy_r1 + enthalpy_r2);

            // Calculate exothermicity (convert from kJ to K using Boltzmann constant and Avogadro's number)
            double exothermicity = -reaction_enthalpy; // Make exothermicity positive for exothermic reactions
            ...
            double probability_on_bare_silicate = 1.0;
            double probability_on_H2O_ice = 1.0;

            if (exothermicity >= 0.0) { // Exothermic reactions

                // Calculate desorption rates for products
                int number_of_atoms_ = 0;
                for (std::size_t idx_p : reaction->product_indices_) {
                ...
                for (std::size_t idx_p : reaction->product_indices_) {
                    ...
                    double eps_m = SQR(120.0 * constants::kProtonMass - ptr_species_manager_->GetSpeciesMass(idx_p)) 
                        / SQR(120.0 * constants::kProtonMass + ptr_species_manager_->GetSpeciesMass(idx_p));

                    double pcb_bare = std::exp(-ptr_species_manager_->GetSpeciesBindingEnergyOnH2Oice(idx_p)
                        / (eps_m * exothermicity / static_cast<double>(number_of_atoms_)));

                    double pcb_ice = 0.1 * pcb_bare;
                ...
                if (ptr_species_manager_->GetSpeciesName(idx_r1) == "sH" && ptr_species_manager_->GetSpeciesName(idx_r2) == "sH") {
                    double pcb_ice = 1.0, pcb_bare = 1.0;
                    probability_on_H2O_ice = (ptr_species_manager_->IsDustSurfaceSpecies(idx_p1)) ? 1.0 - pcb_ice : pcb_ice;
    ...
    void ReactionManager::SetReactionTypeIdStartAndEnd()
    {
        ...
        //               << "Start index = " << std::setw(6) << reaction_type_id_start_[itype] << "   " 
        //               << "End index = " << std::setw(6) << reaction_type_id_end_[itype] << "   "
        //               << "Count = " << std::setw(6) << number_of_each_type_reactions_[itype] << std::endl;
        // }
    }
    ...
    void ReactionManager::SetReactionsInvolvedWithSpecies() 
    {
        ...
        for (std::size_t ispe = 0; ispe < number_of_total_species; ++ispe) {
            ...
            for (std::size_t ireac = 0; ireac < total_number_of_reactions_; ++ireac) {
                ...
                std::size_t idx_r2 = reaction->reactant_indices_[1];
                std::size_t idx_r3 = reaction->reactant_indices_[2];
