...
C***CATEGORY  I1A
C***TYPE      odepack_cpp_real PRECISION (SLSODE-S, DLSODE-D)
...
C       *         LIW, MF
C        odepack_cpp_real PRECISION Y(NEQ), T, TOUT, RTOL, ATOL(ntol), RWORK(LRW)
C
...
C                   INTEGER  NEQ
C                   odepack_cpp_real PRECISION  T, Y(*), YDOT(*)
C
...
C                   odepack_cpp_real PRECISION  T, Y(*), PD(NROWPD,*)
C
...
C       *         MF, NEQ
C        odepack_cpp_real PRECISION  ATOL(3), RTOL, RWORK(58), T, TOUT, Y(3)
C        NEQ = 3
C        Y(1) = 1.D0
C        Y(2) = 0.D0
C        Y(3) = 0.D0
...
C        INTEGER  NEQ
C        odepack_cpp_real PRECISION  T, Y(3), YDOT(3)
...
C        odepack_cpp_real PRECISION  T, Y(3), PD(NRPD,3)
C        PD(1,1) = -.04D0
C        PD(1,2) = 1.D4*Y(3)
C        PD(1,3) = 1.D4*Y(2)
C        PD(2,1) = .04D0
...
C
C     RWORK    A real working array (odepack_cpp_real precision).  The length of
...
C         (218 odepack_cpp_real precision words followed by 37 integer words).
C
...
C           odepack_cpp_real PRECISION RLS
 ...
 */
void Odepack::DLSODE(
    ODEPACK_FUNCTION f, int neq, odepack_cpp_real *y, odepack_cpp_real &t, odepack_cpp_real tout, int itol, odepack_cpp_real *rtol, odepack_cpp_real *atol, int itask,
    int &istate, int iopt, odepack_cpp_real *rwork, int lrw, int *iwork, int liw, ODEPACK_JACOBIAN1 jac, int mf,
    void *user_data)
{
    int i, i1, i2, iflag, imxer, kgo, lf0,
        leniw, lenrw, lenwm, ml, mu;
    odepack_cpp_real atoli, ayi, big, ewti, h0, hmax, hmx, rh, rtoli,
...
LABEL_25:
    ...
    meth = mf / 10;
    miter = mf - 10 * meth;
    if (meth < 1  || meth > 2)  goto LABEL_608;
    if (miter < 0 || miter > 5) goto LABEL_608;
    if (miter <= 3) goto LABEL_30;
    ml = ARRAY1D(iwork, 1);
    mu = ARRAY1D(iwork, 2);
    if (ml < 0 || ml >= n) goto LABEL_609;
    if (mu < 0 || mu >= n) goto LABEL_610;
LABEL_30:
// Next process and check the optional inputs. --------------------------
    if (iopt == 1) goto LABEL_40;
    maxord = ARRAY1D(mord, meth);
    mxstep = mxstp0;
    mxhnil = mxhnl0;
    if (istate == 1) h0 = 0.0;
    hmxi = 0.0;
    ...
    goto LABEL_60;
LABEL_40:
    maxord = ARRAY1D(iwork, 5);
    if (maxord < 0) goto LABEL_611;
    if (maxord == 0) maxord = 100;
    maxord = std::min(maxord, ARRAY1D(mord, meth));
    mxstep = ARRAY1D(iwork, 6);
    if (mxstep < 0) goto LABEL_612;
    if (mxstep == 0) mxstep = mxstp0;
    mxhnil = ARRAY1D(iwork, 7);
    if (mxhnil < 0) goto LABEL_613;
    if (mxhnil == 0) mxhnil = mxhnl0;
    if (istate != 1) goto LABEL_50;
    h0 = ARRAY1D(rwork, 5);
    if ((tout - t) * h0 < 0.0) goto LABEL_614;
LABEL_50:
    hmax = ARRAY1D(rwork, 6);
    if (hmax < 0.0) goto LABEL_615;
    hmxi = 0.0;
    if (hmax > 0.0) hmxi = 1.0 / hmax;
    hmin = ARRAY1D(rwork, 7);
    if (hmin < 0.0) goto LABEL_616;
//-----------------------------------------------------------------------
...
LABEL_60:
    ...
    if (miter == 0) lenwm = 0;
    if (miter == 1 || miter == 2) lenwm = n * n + 2;
...
// If ISTATE = 3, set flag to signal parameter changes to DSTODE. -------
    jstart = -1;
    if (nq <= maxord) goto LABEL_90;
// MAXORD was reduced below NQ.  Copy YH(*,MAXORD+2) into SAVF. ---------
    for (i = 1; i <= n; ++i) {
        ARRAY1D(rwork, i + lsavf - 1) = ARRAY1D(rwork, i + lwm - 1);
    }
// Reload WM(1) = RWORK(LWM), since LWM may have changed. ---------------
LABEL_90:
    if (miter > 0) ARRAY1D(rwork, lwm) = std::sqrt(uround);
    if (n == nyh) goto LABEL_200;
...
// Initial call to F.  (LF0 points to YH(*,2).) -------------------------
    lf0 = lyh + nyh;
    (*f)(neq, t, y, &ARRAY1D(rwork, lf0), user_data);
    nfe = 1;
// Load the initial value vector in yh. --------------------------------
    for (i = 1; i <= n; ++i) {
        ARRAY1D(rwork, i + lyh - 1) = ARRAY1D(y, i);
    }
// Load and invert the EWT array.  (H is temporarily set to 1.0.) -------
    nq = 1;
    h = 1.0;
    DEWSET(n, itol, rtol, atol, &ARRAY1D(rwork, lyh), &ARRAY1D(rwork, lewt));
    for (i = 1; i <= n; ++i) {
        if (ARRAY1D(rwork, i + lewt - 1) <= 0.0) goto LABEL_621;
        ARRAY1D(rwork, i + lewt - 1) = 1.0 / ARRAY1D(rwork, i + lewt - 1);
    }
//-----------------------------------------------------------------------
...
//-----------------------------------------------------------------------
    ...
    if (tdist < 2.0 * uround * w0) goto LABEL_622;
    tol = ARRAY1D(rtol, 1);
    if (itol <= 2) goto LABEL_140;
    for (i = 1; i <= n; ++i) {
        tol = std::max(tol, ARRAY1D(rtol, i));
    }
LABEL_140:
    ...
    for (i = 1; i <= n; ++i) {
        if (itol == 2 || itol == 4) atoli = ARRAY1D(atol, i);
        ayi = std::abs(ARRAY1D(y, i));
        if (ayi != 0.0) tol = std::max(tol, atoli / ayi);
    }
LABEL_160:
    tol = std::max(tol, 100.0 * uround);
    tol = std::min(tol, 0.001);
    sum = DVNORM(n, &ARRAY1D(rwork, lf0), &ARRAY1D(rwork, lewt));
    sum = 1.0 / (tol * w0 * w0) + tol * sum * sum;
    h0 = 1.0 / std::sqrt(sum);
    h0 = std::min(h0, tdist);
    h0 = std::abs(h0) * (tout - t >= 0.0 ? 1.0 : -1.0);
...
LABEL_210:
    if ((tn - tout) * h < 0.0) goto LABEL_250;
    DINTDY(tout, 0, &ARRAY1D(rwork, lyh), nyh, y, iflag);
    if (iflag != 0) goto LABEL_627;
    t = tout;
    goto LABEL_420;
LABEL_220:
    tp = tn - hu * (1.0 + 100.0 * uround);
    if ((tp  - tout) * h > 0.0) goto LABEL_623;
    if ((tn - tout) * h < 0.0) goto LABEL_250;
    goto LABEL_400;
LABEL_230:
    tcrit = ARRAY1D(rwork, 1);
    if ((tn - tcrit) * h > 0.0)  goto LABEL_624;
    if ((tcrit - tout) * h < 0.0) goto LABEL_625;
    if ((tn - tout) * h < 0.0)   goto LABEL_245;
    DINTDY(tout, 0, &ARRAY1D(rwork, lyh), nyh, y, iflag);
    if (iflag != 0) goto LABEL_627;
    t = tout;
    goto LABEL_420;
...
LABEL_245:
    ...
    ihit = std::abs(tn - tcrit) <= (100.0 * uround * hmx);
    if (ihit) goto LABEL_400;
    tnext = tn + h * (1.0 + 4.0 * uround);
    if ((tnext - tcrit) * h <= 0.0) goto LABEL_250;
...
//-----------------------------------------------------------------------
LABEL_250:
    if ((nst - nslast) >= mxstep) goto LABEL_500;
    DEWSET(n, itol, rtol, atol, &ARRAY1D(rwork, lyh), &ARRAY1D(rwork, lewt));
    for (i = 1; i <= n; ++i) {
        if (ARRAY1D(rwork, i + lewt - 1) <= 0.0) goto LABEL_510;
        ARRAY1D(rwork, i + lewt - 1) = 1.0 / ARRAY1D(rwork, i + lewt - 1);
    }
LABEL_270:
    tolsf = uround * DVNORM(n, &ARRAY1D(rwork, lyh), &ARRAY1D(rwork, lewt));
    if (tolsf <= 1.0) goto LABEL_280;
    tolsf *= 2.0;
    if (nst == 0) goto LABEL_626;
    goto LABEL_520;
LABEL_280:
    if ((tn + h) != tn) goto LABEL_290;
    nhnil++;
    if (nhnil > mxhnil) goto LABEL_290;
...
//-----------------------------------------------------------------------
    DSTODE(neq, y, &ARRAY1D(rwork, lyh), nyh, &ARRAY1D(rwork, lyh), &ARRAY1D(rwork, lewt),
        &ARRAY1D(rwork, lsavf), &ARRAY1D(rwork, lacor), &ARRAY1D(rwork, lwm), &ARRAY1D(iwork, liwm),
        f, jac, &Odepack::DPREPJ, &Odepack::DSOLSY, user_data);
    kgo = 1 - kflag;
    if (kgo == 1) {
        goto LABEL_300;
    } else if (kgo == 2) {
...
LABEL_330:
    if ((tn - tout) * h >= 0.0) goto LABEL_400;
    goto LABEL_250;
// ITASK = 4.  See if TOUT or TCRIT was reached.  Adjust H if necessary.
LABEL_340:
    if ((tn - tout) * h < 0.0) goto LABEL_345;
    DINTDY(tout, 0, &ARRAY1D(rwork, lyh), nyh, y, iflag);
    t = tout;
    goto LABEL_420;
LABEL_345:
...
LABEL_400:
    for (i = 1; i <= n; ++i) {
        ARRAY1D(y, i) = ARRAY1D(rwork, i + lyh - 1);
    }
    t = tn;
...
LABEL_420:
    ...
    ARRAY1D(rwork, 12) = h;
    ARRAY1D(rwork, 13) = tn;
    ARRAY1D(iwork, 11) = nst;
    ARRAY1D(iwork, 12) = nfe;
    ARRAY1D(iwork, 13) = nje;
    ARRAY1D(iwork, 14) = nqu;
    ARRAY1D(iwork, 15) = nq;
    return;
//-----------------------------------------------------------------------
// Block H.
...
LABEL_560:
    ...
    imxer = 1;
    for (i = 1; i <= n; ++i) {
        size = std::abs(ARRAY1D(rwork, i + lacor - 1) * ARRAY1D(rwork, i + lewt - 1));
        if (big >= size) continue;
        big = size;
        imxer = i;
    }
    ARRAY1D(iwork, 16) = imxer;
...
LABEL_605:
    ...
    XERRWD(msg, 50, 5, 0, 2, n, neq, 0, 0.0, 0.0);
    goto LABEL_700;
LABEL_606:
...
C
C This version is in odepack_cpp_real precision.
C
...
C               odepack_cpp_real PRECISION T, Y(*), IAN(*), JAN(*), PDJ(*)
...
C                   single precision and 2 in odepack_cpp_real precision).
...
C
C     EXTERNAL FEX, JEX
C     odepack_cpp_real PRECISION ATOL, RTOL, RWORK, T, TOUT, Y
...
C     SUBROUTINE FEX (NEQ, T, Y, YDOT)
C     odepack_cpp_real PRECISION T, Y, YDOT
C     odepack_cpp_real PRECISION RK1, RK2, RK3, RK4, RK5, RK6, RK7, RK8, RK9,
...
C     odepack_cpp_real PRECISION T, Y, PDJ
C     odepack_cpp_real PRECISION RK1, RK2, RK3, RK4, RK5, RK6, RK7, RK8, RK9,
C    1   RK10, RK11, RK12, RK13, RK14, RK15, RK16, RK17
...
C
C RWORK  = a work array used for a mixture of real (odepack_cpp_real precision)
...
C         /DLS001/  of length  255  (218 odepack_cpp_real precision words
C                      followed by 37 integer words),
C         /DLSS01/  of length  40  (6 odepack_cpp_real precision words
...
C           renamed routines for uniqueness across single/odepack_cpp_real prec.;
...
C 20010507  Converted single precision source to odepack_cpp_real precision.
...
C-----------------------------------------------------------------------
 */
void Odepack::DLSODES(
    ODEPACK_FUNCTION f, int neq, odepack_cpp_real *y, odepack_cpp_real &t, odepack_cpp_real tout, int itol, odepack_cpp_real *rtol, odepack_cpp_real *atol, int itask,
    int &istate, int iopt, odepack_cpp_real *rwork, int lrw, int *iwork, int liw, ODEPACK_JACOBIAN2 jac, int mf, void *user_data)
{
    int i, i1, i2, iflag, imax, imul, imxer, ipflag, ipgo, irem,
        j, kgo, lenyht, leniw, lenrw, lf0, lia, lja,
...
// the wordlength for a odepack_cpp_real number to that for an integer.  Usually,
// LENRAT = 1 for single precision and 2 for odepack_cpp_real precision.  If the
...
LABEL_25:
    ...
    meth = mf1 / 10;
    miter = mf1 - 10 * meth;
    if (moss < 0 || moss > 2) goto LABEL_608;
    if (meth < 1 || meth > 2) goto LABEL_608;
    if (miter < 0 || miter > 3) goto LABEL_608;
...
LABEL_50:
    ...
    hmin = ARRAY1D(rwork, 7);
    if (hmin < 0.0) goto LABEL_616;
    seth = ARRAY1D(rwork, 8);
    if (seth < 0.0) goto LABEL_609;
// Check RTOL and ATOL for legality. ------------------------------------
LABEL_60:
    rtoli = ARRAY1D(rtol, 1);
    atoli = ARRAY1D(atol, 1);
...
//-----------------------------------------------------------------------
    ...
    ARRAY1D(iwork, 17) = lenrw;
    leniw = 30;
    if (moss == 0 && miter != 0 && miter != 3) leniw = leniw + n + 1;
    ARRAY1D(iwork, 18) = leniw;
    if (lenrw > lrw) goto LABEL_617;
    if (leniw > liw) goto LABEL_618;
    lia = 31;
    if (moss == 0 && miter != 0 && miter != 3) leniw = leniw + ARRAY1D(iwork, lia + n) - 1;
    ARRAY1D(iwork, 18) = leniw;
    if (leniw > liw) goto LABEL_618;
    lja = lia + n + 1;
    lia = std::min(lia, liw);
    ...
    lwm = 21;
    if (istate == 1) nq = 1;
    ncolm = std::min(nq + 1, maxord + 2);
    lenyhm = ncolm * nyh;
    lenyht = lenyh;
    if (miter == 1 || miter == 2) lenyht = lenyhm;
    imul = 2;
    if (istate == 3) imul = moss;
    if (moss == 2) imul = 3;
    lrtem = lenyht + imul * n;
    lwtem = lwmin;
    if (miter == 1 || miter == 2) lwtem = lrw - 20 - lrtem;
    lenwk = lwtem;
    lyhn = lwm + lwtem;
    lsavf = lyhn + lenyht;
    lewt = lsavf + n;
...
// Move YH.  Move right if LYHD < 0; move left if LYHD > 0. -------------
    if (lyhd < 0) {
        for (i = lyhn; i <= imax; ++i) {
            j = imax + lyhn - i;
            ARRAY1D(rwork, j) = ARRAY1D(rwork, j + lyhd);
        }
    }
    if (lyhd > 0) {
        for (i = lyhn; i <= imax; ++i) {
...
LABEL_80:
    lyh = lyhn;
    ARRAY1D(iwork, 22) = lyh;
    if (miter == 0 || miter == 3) goto LABEL_92;
    if (moss  != 2) goto LABEL_85;
...
// DIPREP and DPREP do sparse matrix preprocessing if MITER = 1 or 2. ---
    lsavf = std::min(lsavf, lrw);
    lewt = std::min(lewt,  lrw);
    lacor = std::min(lacor, lrw);
    DIPREP(neq, y, rwork, &ARRAY1D(iwork, lia), &ARRAY1D(iwork, lja), ipflag, f, jac, user_data);
    lenrw = lwm - 1 + lenwk + lrest;
    ARRAY1D(iwork, 17) = lenrw;
    if (ipflag != -1) ARRAY1D(iwork, 23) = ipian;
    if (ipflag != -1) ARRAY1D(iwork, 24) = ipjan;
    ipgo = -ipflag + 1;
    if (ipgo == 1) {
        goto LABEL_90;
    } else if (ipgo == 2) {
...
// Load and invert the EWT array.  (H is temporarily set to 1.0.) -------
    ...
    if (miter == 0 || miter == 3) goto LABEL_120;
// DIPREP and DPREP do sparse matrix preprocessing if MITER = 1 or 2. ---
    lacor = std::min(lacor, lrw);
    DIPREP(neq, y, rwork,  &ARRAY1D(iwork, lia), &ARRAY1D(iwork, lja), ipflag, f, jac, user_data);
    lenrw = lwm - 1 + lenwk + lrest;
...
//-----------------------------------------------------------------------
    DSTODE(neq, y, &ARRAY1D(rwork, lyh), nyh, &ARRAY1D(rwork, lyh), &ARRAY1D(rwork, lewt), 
        &ARRAY1D(rwork, lsavf), &ARRAY1D(rwork, lacor), &ARRAY1D(rwork, lwm), &ARRAY1D(rwork, lwm), 
...
LABEL_420:
    ...
    ARRAY1D(iwork, 14) = nqu;
    ARRAY1D(iwork, 15) = nq;
    ARRAY1D(iwork, 19) = nnz;
    ARRAY1D(iwork, 20) = ngp;
    ARRAY1D(iwork, 21) = nlu;
    ARRAY1D(iwork, 25) = nzl;
    ARRAY1D(iwork, 26) = nzu;
    return;
//-----------------------------------------------------------------------
// Block H.
// The following block handles all unsuccessful returns other than
...
LABEL_609:
    ...
    XERRWD(msg, 30, 9, 0, 0, 0, 0, 1, seth, 0.0);
    goto LABEL_700;
LABEL_611:
...
LABEL_631:
    ...
    XERRWD(msg, 60, 31, 0, 2, imul, irem, 1, tn, 0.0);
    goto LABEL_700;
LABEL_632:
...
LABEL_633:
    ...
    XERRWD(msg, 60, 33, 0, 2, imul, irem, 1, tn, 0.0);
    if (imul == 2) {
...
C          IOPT = 1 means one or more optional inputs are being used.
C
C RWORK  = a real array (odepack_cpp_real precision) for work space, and (in the
...
C         /DLS001/  of length  255  (218 odepack_cpp_real precision words
C                      followed by 37 integer words),
C         /DLSA01/  of length  31    (22 odepack_cpp_real precision words
...
void Odepack::DLSODA(
    ODEPACK_FUNCTION f, int neq, odepack_cpp_real *y, odepack_cpp_real &t, odepack_cpp_real tout, int itol, odepack_cpp_real *rtol, odepack_cpp_real *atol, int itask,
    int &istate, int iopt, odepack_cpp_real *rwork, int lrw, int *iwork, int liw, ODEPACK_JACOBIAN1 jac, int jt, void *user_data)
{
    int i, i1, i2, iflag, imxer, kgo, lf0,
        leniw, lenrw, lenwm, ml, mu;
...
// DLS001
    ...
    int &init = dls1_.init, &mxstep = dls1_.mxstep, &mxhnil = dls1_.mxhnil, &nhnil = dls1_.nhnil, &nslast = dls1_.nslast, &nyh = dls1_.nyh, 
        ...
        &maxord = dls1_.maxord, &maxcor = dls1_.maxcor, &msbp = dls1_.msbp, &mxncf = dls1_.mxncf, &n = dls1_.n, &nq = dls1_.nq, &nst = dls1_.nst, &nfe = dls1_.nfe, &nje = dls1_.nje, &nqu = dls1_.nqu;
// DLSA01
    odepack_cpp_real &tsw = dlsa_.tsw, &pdnorm = dlsa_.pdnorm;
    int &insufr = dlsa_.insufr, &insufi = dlsa_.insufi, &ixpr = dlsa_.ixpr, &jtyp = dlsa_.jtyp, &mused = dlsa_.mused, &mxordn = dlsa_.mxordn, &mxords = dlsa_.mxords;
//
    const int mxstp0 = 500;
    const int mxhnl0 = 10;
    const int mord[2] = {12, 5};
...
// Next process and check the optional inputs. --------------------------
    ...
    h0 = 0.0;
    mxordn = ARRAY1D(mord, 1);
    mxords = ARRAY1D(mord, 2);
    goto LABEL_60;
LABEL_40:
    ixpr = ARRAY1D(iwork, 5);
    if (ixpr < 0 || ixpr > 1) goto LABEL_611;
    mxstep = ARRAY1D(iwork, 6);
    if (mxstep < 0) goto LABEL_612;
    if (mxstep == 0) mxstep = mxstp0;
    mxhnil = ARRAY1D(iwork, 7);
    ...
    if (istate != 1) goto LABEL_50;
    h0 = ARRAY1D(rwork, 5);
    mxordn = ARRAY1D(iwork, 8);
    if (mxordn < 0) goto LABEL_628;
    if (mxordn == 0) mxordn = 100;
    mxordn = std::min(mxordn, ARRAY1D(mord, 1));
    mxords = ARRAY1D(iwork, 9);
    if (mxords < 0) goto LABEL_629;
    if (mxords == 0) mxords = 100;
    mxords = std::min(mxords, ARRAY1D(mord, 2));
    if ((tout - t) * h0 < 0.0) goto LABEL_614;
LABEL_50:
    hmax = ARRAY1D(rwork, 6);
    if (hmax < 0.0) goto LABEL_615;
...
LABEL_160:
    tol = std::max(tol, 100.0 * uround);
    tol = std::min(tol, 0.001);
    sum = DMNORM(n, &ARRAY1D(rwork, lf0), &ARRAY1D(rwork, lewt));
    sum = 1.0 / (tol * w0 * w0) + tol * sum * sum;
    h0 = 1.0 / std::sqrt(sum);
    h0 = std::min(h0, tdist);
    h0 = std::abs(h0) * (tout - t >= 0.0 ? 1.0 : -1.0);
...
//-----------------------------------------------------------------------
LABEL_250:
    if (meth == mused) goto LABEL_255;
    if (insufr == 1) goto LABEL_550;
    if (insufi == 1) goto LABEL_555;
LABEL_255:
    if ((nst - nslast) >= mxstep) goto LABEL_500;
    DEWSET(n, itol, rtol, atol, &ARRAY1D(rwork, lyh), &ARRAY1D(rwork, lewt));
    for (i = 1; i <= n; ++i) {
...
LABEL_270:
    tolsf = uround * DMNORM(n, &ARRAY1D(rwork, lyh), &ARRAY1D(rwork, lewt));
    if (tolsf <= 1.0) goto LABEL_280;
    tolsf *= 2.0;
    if (nst == 0) goto LABEL_626;
    goto LABEL_520;
...
//-----------------------------------------------------------------------
    DSTODA(neq, y, &ARRAY1D(rwork, lyh), nyh, &ARRAY1D(rwork, lyh), &ARRAY1D(rwork, lewt), 
        &ARRAY1D(rwork, lsavf), &ARRAY1D(rwork, lacor), &ARRAY1D(rwork, lwm), &ARRAY1D(iwork, liwm), 
...
LABEL_300:
    init = 1;
    if (meth == mused) goto LABEL_310;
    tsw = tn;
    maxord = mxordn;
    if (meth == 2) maxord = mxords;
    if (meth == 2) ARRAY1D(rwork, lwm) = std::sqrt(uround);
    insufr = std::min(insufr, 1);
    insufi = std::min(insufi, 1);
    jstart = -1;
    if (ixpr == 0) goto LABEL_310;
    if (meth == 2) {
...
// ITASK = 1.  If TOUT has been reached, interpolate. -------------------
LABEL_320:
    // std::cout << nst << " " << std::scientific 
    //     << ARRAY1D(y, 1) << " "
    //     << ARRAY1D(y, 2) << " "
    //     << ARRAY1D(y, 3) << std::endl;
    if ((tn - tout) * h < 0.0) goto LABEL_250;
    DINTDY(tout, 0, &ARRAY1D(rwork, lyh) , nyh, y, iflag);
    t = tout;
    goto LABEL_420;
...
LABEL_420:
    ...
    ARRAY1D(rwork, 12) = h;
    ARRAY1D(rwork, 13) = tn;
    ARRAY1D(rwork, 15) = tsw;
    ARRAY1D(iwork, 11) = nst;
    ARRAY1D(iwork, 12) = nfe;
    ARRAY1D(iwork, 13) = nje;
    ARRAY1D(iwork, 14) = nqu;
    ARRAY1D(iwork, 15) = nq;
    ARRAY1D(iwork, 19) = mused;
    ARRAY1D(iwork, 20) = meth;
    return;
//-----------------------------------------------------------------------
// Block H.
// The following block handles all unsuccessful returns other than
...
LABEL_550:
    ...
    XERRWD(msg, 60, 207, 0, 0, 0, 0, 1, tn, 0.0);
    istate = -7;
    goto LABEL_580;
...
LABEL_611:
    ...
    XERRWD(msg, 30, 11, 0, 1, ixpr, 0, 0, 0.0, 0.0);
    goto LABEL_700;
LABEL_612:
...
LABEL_628:
    ...
    XERRWD(msg, 30, 28, 0, 1, mxordn, 0, 0, 0.0, 0.0);
    goto LABEL_700;
LABEL_629:
    msg = "DLSODA-  mxords (=I1) .lt. 0  ";
    XERRWD(msg, 30, 29, 0, 1, mxords, 0, 0, 0.0, 0.0);
//
LABEL_700:
    istate = -3;
    return;
...
C               odepack_cpp_real PRECISION T, Y(*), GOUT(NG)
...
C     odepack_cpp_real PRECISION T, Y, GOUT
...
C         /DLSA01/  of length  31    (22 odepack_cpp_real precision words
C                      followed by  9 integer words).
C         /DLSR01/  of length   7  (3 odepack_cpp_real precision words
...
void Odepack::DLSODAR(
    ...
    void *user_data)
{
    int i, i1, i2, iflag, imxer, kgo, leniw,
...
// DLSR01
    ...
    int &lg0 = dlsr_.lg0, &lg1 = dlsr_.lg1, &lgx = dlsr_.lgx, &irfnd = dlsr_.irfnd, &itaskc = dlsr_.itaskc, &ngc = dlsr_.ngc, &nge = dlsr_.nge;
//
    const int mord[2] = {12, 5};
    const int mxstp0  = 500;
    const int mxhnl0  = 10;
...
LABEL_30:
    ...
    if (irfnd == 0 && ng != ngc) goto LABEL_631;
LABEL_35:
    ngc = ng;
// Next process and check the optional inputs. --------------------------
    if (iopt == 1) goto LABEL_40;
...
LABEL_160:
    ...
    h0 = 1.0 / std::sqrt(sum);
    h0 = std::min(h0, tdist);
    h0 = std::abs(h0) * ((tout - t) >= 0 ? 1.0: -1.0);
// Adjust H0 if necessary to meet HMAX bound. ---------------------------
LABEL_180:
    rh = std::abs(h0) * hmxi;
    if (rh > 1.0) h0 /= rh;
...
// Check for a zero of g at T. ------------------------------------------
    irfnd = 0;
    toutc = tout;
    if (ngc == 0) goto LABEL_270;
    DRCHEK(1, g, neq, y, &ARRAY1D(rwork, lyh), nyh,
        &ARRAY1D(rwork, lg0), &ARRAY1D(rwork, lg1), &ARRAY1D(rwork, lgx), jroot, irt, user_data);
...
//
    irfp = irfnd;
    if (ngc == 0) goto LABEL_205;
    if (itask == 1 || itask == 4) toutc = tout;
    DRCHEK(2, g, neq, y, &ARRAY1D(rwork, lyh), nyh, 
        &ARRAY1D(rwork, lg0), &ARRAY1D(rwork, lg1), &ARRAY1D(rwork, lgx), jroot, irt, user_data);
...
LABEL_205:
    irfnd = 0;
    if (irfp == 1 && tlast != tn && itask == 2) goto LABEL_400;
//
    if (itask == 1) {
        goto LABEL_210;
    } else if (itask == 2) {
...
LABEL_245:
    ...
    ihit = std::abs(tn - tcrit) <= 100.0 * uround * hmx;
    if (ihit) t = tcrit;
    if (irfp == 1 && tlast != tn && itask == 5) goto LABEL_400;
    if (ihit) goto LABEL_400;
    tnext = tn + h * (1.0 + 4.0 * uround);
    if ((tnext - tcrit) * h <= 0.0) goto LABEL_250;
    h = (tcrit - tn) * (1.0 - 4.0 * uround);
...
LABEL_290:
//-----------------------------------------------------------------------
//   DSTODA(NEQ,Y,YH,nyh,YH,EWT,SAVF,ACOR,WM,IWM,F,JAC,DPRJA,DSOLSY)
//-----------------------------------------------------------------------
    DSTODA(neq, y, &ARRAY1D(rwork, lyh), nyh, &ARRAY1D(rwork, lyh), &ARRAY1D(rwork, lewt),
        &ARRAY1D(rwork, lsavf), &ARRAY1D(rwork, lacor), &ARRAY1D(rwork, lwm), &ARRAY1D(iwork, liwm),
        f, jac, &Odepack::DPRJA, &Odepack::DSOLSY, user_data);
...
LABEL_310:
//
    if (ngc == 0) goto LABEL_315;
    DRCHEK(3, g, neq, y, &ARRAY1D(rwork, lyh), nyh,
        &ARRAY1D(rwork, lg0), &ARRAY1D(rwork, lg1), &ARRAY1D(rwork, lgx), jroot, irt, user_data);
...
LABEL_550:
    ...
    XERRWD(msg, 50, 206, 0, 0, 0, 0, 0, 0.0, 0.0);
    msg = "      to proceed.  The integration was otherwise successful.";
    XERRWD(msg, 60, 206, 0, 0, 0, 0, 1, tn, 0.0);
    istate = -7;
    goto LABEL_580;
// IWORK length too small to proceed. -----------------------------------
LABEL_555:
...
LABEL_631:
    ...
    XERRWD(msg, 50, 31, 0, 2, ngc, ng, 0, 0.0, 0.0);
    goto LABEL_700;
LABEL_632:
...
C     odepack_cpp_real PRECISION T, Y(*),YSV(*), REWT(*), FTY(*), V(*), HL0, WP(*)
C     INTEGER IWP(*)
...
C     odepack_cpp_real PRECISION T, Y(*), FTY(*), WK(*), HL0, WP(*), B(*)
C     INTEGER IWP(*)
...
C          IOPT = 1 means one or more optional inputs are being used.
C
C RWORK  = a real working array (odepack_cpp_real precision).
...
C           1                HL0, WP, IWP, IER)
C            odepack_cpp_real PRECISION T, Y(*),YSV(*), REWT(*), FTY(*), V(*),
C           1                 HL0, WP(*)
C            INTEGER IWP(*)
C          This routine must evaluate and preprocess any parts of the
...
C         /DLS001/  of length  255  (218 odepack_cpp_real precision words
C                      followed by 37 integer words),
C         /DLPK01/  of length  17  (4 odepack_cpp_real precision words
...
C-----------------------------------------------------------------------
 */
void Odepack::DLSODPK(
    ODEPACK_FUNCTION f, int neq, odepack_cpp_real *y, odepack_cpp_real &t, odepack_cpp_real tout, int itol, odepack_cpp_real *rtol, odepack_cpp_real *atol, int itask,
    int &istate, int iopt, odepack_cpp_real *rwork, int lrw, int *iwork, int liw, ODEPACK_JACOBIAN3 jac, ODEPACK_PSOL psol, int mf, void *user_data)
{
    int i, i1, i2, iflag, imxer, kgo, lf0, leniw,
        leniwk, lenrw, lenwm, lenwk, liwp, lwp,
        ncfn0, ncfl0, nli0, nni0, nnid, nstd, nwarn;
    odepack_cpp_real atoli, avdim, ayi, big, ewti, h0, hmax, hmx,
        rcfl, rcfn, rh, rtoli, tcrit,
        tdist, tnext, tol, tolsf, tp, size, sum, w0;
    bool ihit, lavd, lcfn, lcfl, lwarn;
    std::string msg;
// DLS001
    odepack_cpp_real &ccmax = dls1_.ccmax, &el0 = dls1_.el0, &h = dls1_.h, &hmin = dls1_.hmin, &hmxi = dls1_.hmxi, &hu = dls1_.hu, &rc = dls1_.rc, &tn = dls1_.tn, &uround = dls1_.uround;
    int &init = dls1_.init, &mxstep = dls1_.mxstep, &mxhnil = dls1_.mxhnil, &nhnil = dls1_.nhnil, &nslast = dls1_.nslast, &nyh = dls1_.nyh, 
...
LABEL_25:
    ...
    miter = mf - 10 * meth;
    if (meth < 1 || meth > 2) goto LABEL_608;
    if (miter < 0) goto LABEL_608;
    if (miter > 4 && miter < 9) goto LABEL_608;
    if (miter >= 1) jpre = ARRAY1D(iwork, 3);
    jacflg = 0;
    if (miter >= 1) jacflg = ARRAY1D(iwork, 4);
// Next process and check the optional inputs. --------------------------
    if (iopt == 1) goto LABEL_40;
    maxord = ARRAY1D(mord, meth);
    mxstep = mxstp0;
...
LABEL_50:
    ...
    hmin = ARRAY1D(rwork, 7);
    if (hmin < 0.0) goto LABEL_616;
    maxl = ARRAY1D(iwork, 8);
    if (maxl == 0) maxl = 5;
    maxl = std::min(maxl, n);
    kmp = ARRAY1D(iwork, 9);
    if (kmp == 0 || kmp > maxl) kmp = maxl;
    delt = ARRAY1D(rwork, 8);
...
LABEL_60:
    ...
    if (miter == 2) lenwk = n * (maxl + std::min(1, maxl - kmp)) + (maxl + 3) * maxl;
    if (miter == 3 || miter == 4) lenwk = 5 * n;
    if (miter == 9) lenwk = 2 * n;
    lwp = 0;
    if (miter >= 1) lwp = ARRAY1D(iwork, 1);
    lenwm = lenwk + lwp;
    locwp = lenwk + 1;
    lewt = lwm + lenwm;
    lsavf = lewt + n;
    ...
    liwp = 0;
    if (miter >= 1) liwp = ARRAY1D(iwork, 2);
...
// Load SQRT(N) and its reciprocal in Common. ---------------------------
    sqrtn = std::sqrt(static_cast<odepack_cpp_real >(n));
    rsqrtn = 1.0 / sqrtn;
    if (istate == 1) goto LABEL_100;
...
LABEL_250:
    ...
    nstd = nst - nslast;
    nnid = nni - nni0;
    if (nstd < 10 || nnid == 0) goto LABEL_255;
    avdim = static_cast<odepack_cpp_real >(nli - nli0) / static_cast<odepack_cpp_real >(nnid);
    rcfn = static_cast<odepack_cpp_real >(ncfn - ncfn0) / static_cast<odepack_cpp_real >(nstd);
    rcfl = static_cast<odepack_cpp_real >(ncfl - ncfl0) / static_cast<odepack_cpp_real >(nnid);
    lavd = (avdim > (static_cast<odepack_cpp_real >(maxl) - 0.05));
    lcfn = (rcfn > 0.9);
    lcfl = (rcfl > 0.9);
    lwarn = (lavd || lcfn || lcfl);
    if (!lwarn) goto LABEL_255;
    nwarn++;
    if (nwarn > 10) goto LABEL_255;
    if (lavd) {
        msg = "DLSODPK- Warning. Poor iterative algorithm performance seen ";
        XERRWD(msg, 60, 111, 0, 0, 0, 0, 0, 0.0, 0.0);
    }
    if (lavd) {
        msg = "DLSODPK- Warning. Poor iterative algorithm performance seen ";
        XERRWD(msg, 60, 112, 0, 0, 0, 0, 0, 0.0, 0.0);
    }
    if (lcfn) {
        msg = "DLSODPK- Warning. Poor iterative algorithm performance seen ";
        XERRWD(msg, 60, 112, 0, 0, 0, 0, 0, 0.0, 0.0);
    ...
    if (lcfl) {
        msg = "DLSODPK- Warning. Poor iterative algorithm performance seen ";
        XERRWD(msg, 60, 113, 0, 0, 0, 0, 0, 0.0, 0.0);
    }
    if (lcfl) {
...
//-----------------------------------------------------------------------
    DSTODPK(neq, y, &ARRAY1D(rwork, lyh), nyh, &ARRAY1D(rwork, lyh), &ARRAY1D(rwork, lewt),
        &ARRAY1D(rwork, lsavf), &ARRAY1D(rwork, lsavx), &ARRAY1D(rwork, lacor), &ARRAY1D(rwork, lwm),
        &ARRAY1D(iwork, liwm), f, jac, psol, user_data);
    kgo = 1 - kflag;
    if (kgo == 1) {
        goto LABEL_300;
    } else if (kgo == 2) {
...
LABEL_345:
    ...
    if (ihit) goto LABEL_400;
    tnext = tn + h * (1.0 + 4.0 * uround);
    if ((tnext - tn) * h <= 0.0) goto LABEL_250;
    h = (tcrit - tn) * (1.0 - 4.0 * uround);
    jstart = -2;
    goto LABEL_250;
// ITASK = 5.  see if TCRIT was reached and jump to exit. ---------------
...
LABEL_420:
    ...
    ARRAY1D(iwork, 14) = nqu;
    ARRAY1D(iwork, 15) = nq;
    ARRAY1D(iwork, 19) = nni;
    ARRAY1D(iwork, 20) = nli;
    ARRAY1D(iwork, 21) = nps;
    ARRAY1D(iwork, 22) = ncfn;
    ARRAY1D(iwork, 23) = ncfl;
    return;
//-----------------------------------------------------------------------
// Block H.
// The following block handles all unsuccessful returns other than
...
// KFLAG = -1.  Error test failed repeatedly or with ABS(H) = HMIN. -----
LABEL_530:
    msg = "DLSODPK-  At T(=R1), step size H(=R2), the error  ";
    XERRWD(msg, 50, 204, 0, 0, 0, 0, 0, 0.0, 0.0);
    msg = "      test failed repeatedly or with ABS(H) = HMIN'";
    XERRWD(msg, 50, 204, 0, 0, 0, 0, 2, tn, h);
    istate = -4;
...
// KFLAG = -2.  Convergence failed repeatedly or with ABS(H) = HMIN. ----
LABEL_540:
    msg = "DLSODPK-  At T (=R1) and step size H (=R2), the   ";
    XERRWD(msg, 50, 205, 0, 0, 0, 0, 0, 0.0, 0.0);
    msg = "      corrector convergence failed repeatedly     ";
    XERRWD(msg, 50, 205, 0, 0, 0, 0, 0, 0.0, 0.0);
    msg = "      or with ABS(H) = HMIN   ";
...
LABEL_550:
    msg = "DLSODPK-  At T (=R1) an unrecoverable error return";
    XERRWD(msg, 50, 205, 0, 0, 0, 0, 0, 0.0, 0.0);
    msg = "      was made from Subroutine PSOL     ";
    XERRWD(msg, 40, 205, 0, 0, 0, 0, 1, tn, 0.0);
    istate = -7;
    goto LABEL_580;
// Compute IMXER if relevant. -------------------------------------------
LABEL_560:
...
LABEL_602:
    ...
    goto LABEL_700;
LABEL_603:
    msg = "DLSODPK-  ISTATE.gt.1 but DLSODPK not initialized.";
    XERRWD(msg, 50, 3, 0, 0, 0, 0, 0, 0.0, 0.0);
    goto LABEL_700;
LABEL_604:
    ...
    goto LABEL_700;
LABEL_605:
    msg = "DLSODPK-  ISTATE = 3 and NEQ increased (I1 to I2).";
    XERRWD (msg, 50, 5, 0, 2, n, neq, 0, 0.0, 0.0);
    goto LABEL_700;
LABEL_606:
...
LABEL_613:
    ...
    goto LABEL_700;
LABEL_614:
    msg = "DLSODPK-  TOUT (=R1) behind T (=R2)     ";
    XERRWD(msg, 40, 14, 0, 0, 0, 0, 2, tout, t);
    msg = "      Integration direction is given by H0 (=R1)  ";
    XERRWD(msg, 50, 14, 0, 0, 0, 0, 1, h0, 0.0);
    goto LABEL_700;
LABEL_615:
    ...
    goto LABEL_700;
LABEL_616:
    msg = "DLSODPK-  HMIN (=R1) .lt. 0.0 ";
    XERRWD(msg, 30, 16, 0, 0, 0, 0, 1, hmin, 0.0);
    goto LABEL_700;
LABEL_617:
    msg = "DLSODPK-  RWORK length needed, LENRW(=I1), exceeds LRW(=I2) ";
    XERRWD(msg, 60, 17, 0, 2, lenrw, lrw, 0, 0.0, 0.0);
    goto LABEL_700;
LABEL_618:
    msg = "DLSODPK-  IWORK length needed, LENIW(=I1), exceeds LIW(=I2) ";
    XERRWD(msg, 60, 18, 0, 2, leniw, liw, 0, 0.0, 0.0);
    goto LABEL_700;
LABEL_619:
    msg = "DLSODPK-  RTOL(I1) is R1 .lt. 0.0       ";
    XERRWD(msg, 40, 19, 0, 1, i, 0, 1, rtoli, 0.0);
    goto LABEL_700;
LABEL_620:
    msg = "DLSODPK-  ATOL(I1) is R1 .lt. 0.0       ";
    XERRWD(msg, 40, 20, 0, 1, i, 0, 1, atoli, 0.0);
    goto LABEL_700;
LABEL_621:
    ewti = ARRAY1D(rwork, i + lewt - 1);
    ...
    goto LABEL_700;
LABEL_622:
    msg = "DLSODPK- TOUT(=R1) too close to T(=R2) to start integration.";
    XERRWD(msg, 60, 22, 0, 0, 0, 0, 2, tout, t);
    goto LABEL_700;
LABEL_623:
    msg = "DLSODPK-  ITASK = I1 and TOUT (=R1) behind TCUR - HU (= R2) ";
    XERRWD(msg, 60, 23, 0, 1, itask, 0, 2, tout, tp);
    goto LABEL_700;
LABEL_624:
    msg = "DLSODPK-  ITASK = 4 or 5 and TCRIT (=R1) behind TCUR (=R2)  ";
    XERRWD(msg, 60, 24, 0, 0, 0, 0, 2, tcrit, tn);
    goto LABEL_700;
LABEL_625:
    msg = "DLSODPK-  ITASK = 4 or 5 and TCRIT (=R1) behind TOUT (=R2)  ";
    XERRWD(msg, 60, 25, 0, 0, 0, 0, 2, tcrit, tout);
    goto LABEL_700;
LABEL_626:
    ...
    goto LABEL_700;
LABEL_627:
    msg = "DLSODPK-  Trouble in DINTDY. ITASK = I1, TOUT = R1";
    XERRWD(msg, 50, 27, 0, 1, itask, 0, 1, tout, 0.0);
//
LABEL_700:
    istate = -3;
...
C         /DLS001/  of length  255  (218 odepack_cpp_real precision words
C                      followed by 37 integer words),
C         /DLS002/  of length   5  (1 odepack_cpp_real precision word
C                      followed by  4 integer words),
C         /DLPK01/  of length  17  (4 odepack_cpp_real precision words
C                      followed by 13 integer words),
C         /DLSR01/  of length  14     (5 odepack_cpp_real precision words
C                      followed by  9 integer words).
C
...
{
    int i, i1, i2, ier, iflag, imxer, kgo, lf0, leniw, leniwk, lenrw, lenwm, lenwk, liwp, lwp, ncfn0, ncfl0, niter, nli0, nni0, nnid, nstd, nwarn;
    int irfp, irt, lenyh, lyhnew;
    odepack_cpp_real atoli, avdim, big, ewti, h0, hmax, hmx, rcfl, rcfn, rh, rtoli, tcrit, tnext, tolsf, tp, size;
    bool ihit, lavd, lcfn, lcfl, lwarn;
    std::string msg;
// DLS001
    odepack_cpp_real &ccmax = dls1_.ccmax, &el0 = dls1_.el0, &h = dls1_.h, &hmin = dls1_.hmin, &hmxi = dls1_.hmxi, &hu = dls1_.hu, &rc = dls1_.rc, &tn = dls1_.tn, &uround = dls1_.uround;
...
LABEL_25:
    ...
    if (miter < 0) goto LABEL_608;
    if (miter > 4 && miter < 9) goto LABEL_608;
    if (miter >= 1) jpre = ARRAY1D(iwork, 4);
    if (ng < 0) goto LABEL_630;
    if (istate == 1) goto LABEL_35;
    if (irfnd == 0 && ng != ngc) goto LABEL_631;
LABEL_35:
...
LABEL_62:
    ...
    if (miter == 0) lenwk = 0;
    if (miter == 1) lenwk = n * (maxl + 2) + maxl * maxl;
    if (miter == 2) lenwk = n * (maxl + 2 + std::min(1, maxl - kmp)) + (maxl + 3) * maxl + 1;
    if (miter == 3 || miter == 4) lenwk = 5 * n;
    if (miter == 9) lenwk = 2 * n;
    lwp = 0;
    if (miter >= 1) lwp = ARRAY1D(iwork, 1);
...
// Call DLHIN to set initial step size H0 to be attempted. --------------
    DLHIN(neq, n, t, &ARRAY1D(rwork, lyh), &ARRAY1D(rwork, lf0), f, tout, uround,
        &ARRAY1D(rwork, lewt), itol, atol, y, &ARRAY1D(rwork, lacor), h0, niter, ier, user_data);
    nfe += niter;
    if (ier != 0) goto LABEL_622;
// Adjust H0 if necessary to meet HMAX bound. ---------------------------
LABEL_180:
    rh = std::abs(h0) * hmxi;
    if (rh > 1.0) h0 /= rh;
...
//-----------------------------------------------------------------------
    DSTOKA(neq, y, &ARRAY1D(rwork, lyh), nyh, &ARRAY1D(rwork, lyh), &ARRAY1D(rwork, lewt),
        &ARRAY1D(rwork, lsavf), &ARRAY1D(rwork, lsavx), &ARRAY1D(rwork, lacor), &ARRAY1D(rwork, lwm),
        &ARRAY1D(iwork, liwm), f, jac, psol, user_data);
    kgo = 1 - kflag;
    if (kgo == 1) {
...
LABEL_425:
    ...
    ARRAY1D(iwork, 22) = ncfn;
    ARRAY1D(iwork, 23) = ncfl;
    ARRAY1D(iwork, 24) = nsfi;
    ARRAY1D(iwork, 25) = njev;
    ARRAY1D(iwork, 10) = nge;
    tlast = t;
    return;
//-----------------------------------------------------------------------
...
LABEL_550:
    ...
    XERRWD(msg, 50, 206, 0, 0, 0, 0, 0, 0.0, 0.0);
    msg = "      was made from Subroutine PSOL     ";
    XERRWD(msg, 40, 206, 0, 0, 0, 0, 1, tn, 0.0);
    istate = -7;
    goto LABEL_580;
// Compute IMXER if relevant. -------------------------------------------
LABEL_560:
...
C               odepack_cpp_real PRECISION T, Y(*), S(*), R(*)
...
C               odepack_cpp_real PRECISION T, Y(*), P(NROWP,*)
...
C               odepack_cpp_real PRECISION T, Y(*), S(*), P(NROWP,*)
...
C     odepack_cpp_real PRECISION ATOL, RTOL, RWORK, T, TOUT, Y, YDOTI
...
C     odepack_cpp_real PRECISION T, Y, P
...
C     odepack_cpp_real PRECISION T, Y, S, P
...
{
    int i, i1, i2, ier, iflag, imxer, ires, kgo,
        leniw, lenrw, lenwm, lp, lyd0, ml, mu;
    odepack_cpp_real atoli, ayi, big, ewti, h0, hmax, hmx, rh, rtoli,
        tcrit, tdist, tnext, tol, tolsf, tp, size, sum, w0;
    bool ihit;
    std::string msg;
...
//-----------------------------------------------------------------------
    if (istate < 0 || istate > 3) goto LABEL_601;
    if (itask < 1 || itask > 5) goto LABEL_602;
    if (istate <= 1) goto LABEL_10;
    if (init == 0) goto LABEL_603;
    if (istate == 2) goto LABEL_200;
...
// MAXORD was reduced below NQ.  Copy YH(*,MAXORD+2) into YDOTI.---------
    for (i = 1; i <= n; ++i) {
        ARRAY1D(ydoti, i) = ARRAY1D(rwork, i + lwm - 1);
    }
// Reload WM(1) = RWORK(lWM), since lWM may have changed. ---------------
LABEL_90:
    ARRAY1D(rwork, lwm) = std::sqrt(uround);
    if (n == nyh) goto LABEL_200;
// NEQ was reduced.  Zero part of YH to avoid undefined references. -----
    i1 = lyh + l * nyh;
    i2 = lyh + (maxord + 1) * nyh - 1;
...
LABEL_100:
    uround = DUMACH();
    tn = t;
    if (itask != 4 && itask != 5) goto LABEL_105;
    tcrit = ARRAY1D(rwork, 1);
    if ((tcrit - tout) * (tout - t) < 0.0) goto LABEL_625;
    if (h0 != 0.0 && (t + h0 - tcrit) * h0 > 0.0) h0 = tcrit - t;
LABEL_105:
...
// Compute initial dy/dt, if necessary, and load it and initial Y into YH
    lyd0 = lyh + nyh;
    lp = lwm + 1;
    if (istate == 1) goto LABEL_120;
// DLSODI must compute initial dy/dt (LYD0 points to YH(*,2)). ----------
    DAINVG(res, adda, neq, t, y, &ARRAY1D(rwork, lyd0), miter,
        ml, mu, &ARRAY1D(rwork, lp), &ARRAY1D(iwork, 21), ier, user_data);
    nfe++;
    if (ier < 0) goto LABEL_560;
    if (ier > 0) goto LABEL_565;
    for (i = 1; i <= n; ++i) {
        ARRAY1D(rwork, i + lyh - 1) = ARRAY1D(y, i);
    }
    goto LABEL_130;
...
LABEL_120:    
    for (i = 1; i <= n; ++i) {
        ARRAY1D(rwork, i + lyh - 1) = ARRAY1D(y, i);
        ARRAY1D(rwork, i + lyd0 - 1) = ARRAY1D(ydoti, i);
    }
// Load and invert the EWT array.  (H is temporarily set to 1.0.) -------
LABEL_130:
    nq = 1;
...
LABEL_160:
    tol = std::max(tol, 100.0 * uround);
    tol = std::min(tol, 0.001);
    sum = DVNORM(n, &ARRAY1D(rwork, lyd0), &ARRAY1D(rwork, lewt));
    sum = 1.0 / (tol * w0 * w0) + tol * sum * sum;
    h0 = 1.0 / std::sqrt(sum);
    h0 = std::min(h0, tdist);
    h0 = std::abs(h0) * (tout - t >= 0.0 ? 1.0 : -1.0);
...
//-----------------------------------------------------------------------
    DSTODI(neq, y, &ARRAY1D(rwork, lyh), nyh, &ARRAY1D(rwork, lyh), &ARRAY1D(rwork, lewt),
        ydoti, &ARRAY1D(rwork, lsavf), &ARRAY1D(rwork, lacor), &ARRAY1D(rwork, lwm),
        &ARRAY1D(iwork, liwm), res, adda, jac, &Odepack::DPREPJI, &Odepack::DSOLSY, user_data);
    kgo = 1 - kflag;
    if (kgo == 1) {
        goto LABEL_300;
    } else if (kgo == 2) {
...
LABEL_580:
    lyd0 = lyh + nyh;
    for (i = 1; i <= n; ++i) {
        ARRAY1D(rwork, i + lsavf - 1) = ARRAY1D(rwork, i + lyd0 - 1) / h;
        ARRAY1D(y, i) = ARRAY1D(rwork, i + lyh - 1);
    }
    ires = 1;
    (*res)(neq, tn, y, &ARRAY1D(rwork, lsavf), ydoti, ires, user_data);
    nfe++;
    if (ires <= 1) goto LABEL_595;
    msg = "DLSODI-  Residual routine set its flag IRES       ";
    XERRWD(msg, 50, 210, 0, 0, 0, 0, 0, 0.0, 0.0);
    msg = "      to (I1) when called for final output.       ";
    XERRWD(msg, 50, 210, 0, 1, ires, 0, 0, 0.0, 0.0);
    goto LABEL_595;
// Set Y vector, T, and optional outputs. -------------------------------
LABEL_590:
    for (i = 1; i <= n; ++i) {
...
C     odepack_cpp_real PRECISION T, Y(*), PA(MB,MB,NB), PB(MB,MB,NB), PC(MB,MB,NB)
...
C     odepack_cpp_real PRECISION T, Y(*), S(*), PA(MB,MB,NB), PB(MB,MB,NB),
C    1                 PC(MB,MB,NB)
C which computes dr/dy as a function of t, y, and s.  Here T, Y, and
...
C     odepack_cpp_real PRECISION T, Y, S, R, ETA, DELX, EODSQ
...
C     odepack_cpp_real PRECISION T, Y, PA, PB, PC
...
C     odepack_cpp_real PRECISION T, Y, S, PA, PB, PC, ETA, DELX, EODSQ
...
C               SUBROUTINE ADDA (NEQ, T, Y, MB, NB, PA, PB, PC)
C               odepack_cpp_real PRECISION T, Y(*), PA(MB,MB,NB), PB(MB,MB,NB),
C              1                 PC(MB,MB,NB)
...
C          passed.  This subroutine is to have the form
C               SUBROUTINE JAC (NEQ, T, Y, S, MB, NB, PA, PB, PC)
C               odepack_cpp_real PRECISION T, Y(*), S(*), PA(MB,MB,NB),
...
//
    int i, i1, i2, ier, iflag, imxer, ires, kgo,
        leniw, lenrw, lenwm, lp, lyd0, mb, nb;
    odepack_cpp_real atoli, ayi, big, ewti, h0, hmax, hmx, rh, rtoli,
        tcrit, tdist, tnext, tol, tolsf, tp, size, sum, w0;
    bool ihit;
    std::string msg;
...
LABEL_25:
    ...
    miter = mf - 10 * meth;
    if (meth < 1 || meth > 2) goto LABEL_608;
    if (miter < 1 || miter > 2) goto LABEL_608;
    mb = ARRAY1D(iwork, 1);
    nb = ARRAY1D(iwork, 2);
    if (mb < 1 || mb > n) goto LABEL_609;
    if (nb < 4) goto LABEL_610;
    if (mb * nb != n) goto LABEL_609;
// Next process and check the optional inputs. --------------------------
    if (iopt == 1) goto LABEL_40;
    maxord = ARRAY1D(mord, meth);
    mxstep = mxstp0;
...
// DLSOIBT must compute initial dy/dt (LYD0 points to YH(*,2)). ---------
    DAIGBT(res, adda, neq, t, y, &ARRAY1D(rwork, lyd0),
        mb, nb, &ARRAY1D(rwork, lp), &ARRAY1D(iwork, 21), ier, user_data);
    nfe++;
    if (ier < 0) goto LABEL_560;
    if (ier > 0) goto LABEL_565;
    for (i = 1; i <= n; ++i) {
...
//-----------------------------------------------------------------------
    DSTODI(neq, y, &ARRAY1D(rwork, lyh), nyh, &ARRAY1D(rwork, lyh), &ARRAY1D(rwork, lewt),
        ydoti, &ARRAY1D(rwork, lsavf), &ARRAY1D(rwork, lacor), &ARRAY1D(rwork, lwm),
        &ARRAY1D(iwork, liwm), res, adda, jac, &Odepack::DPJIBT, &Odepack::DSLSBT, user_data);
    kgo = 1 - kflag;
    if (kgo == 1) {
        goto LABEL_300;
    } else if (kgo == 2) {
...
// KFLAG = -1.  Error test failed repeatedly or with ABS(H) = HMIN. -----
LABEL_530:
    msg = "DLSOIBT- At T (=R1) and step size H (=R2), the    ";
    XERRWD(msg, 50, 204, 0, 0, 0, 0, 0, 0.0, 0.0);
...
// IRES = 3 returned by RES, despite retries by DSTODI.------------------
LABEL_550:
    msg = "DLSOIBT- At T (=R1) residual routine returned     ";
    XERRWD(msg, 50, 206, 0, 0, 0, 0, 0, 0.0, 0.0);
    msg = "      error IRES = 3 repeatedly.        ";
    XERRWD(msg, 40, 206, 0, 0, 0, 0, 1, tn, 0.0);
    istate = -7;
...
LABEL_565:
    msg = "DLSOIBT- Attempt to initialize dy/dt failed       ";
    XERRWD(msg, 50, 208, 0, 0, 0, 0, 0, 0.0, 0.0);
    msg = "      because residual routine set its error flag ";
    XERRWD(msg, 50, 208, 0, 0, 0, 0, 0, 0.0, 0.0);
    msg = "      to IRES = (I1)";
...
LABEL_602:
    ...
    goto LABEL_700;
LABEL_603:
    msg = "DLSOIBT- ISTATE.gt.1 but DLSOIBT not initialized. ";
    XERRWD(msg, 50, 3, 0, 0, 0, 0, 0, 0.0, 0.0);
    goto LABEL_700;
LABEL_604:
    ...
    goto LABEL_700;
LABEL_605:
    msg = "DLSOIBT- ISTATE = 3 and NEQ increased (I1 to I2). ";
    XERRWD(msg, 50, 5, 0, 2, n, neq, 0, 0.0, 0.0);
    goto LABEL_700;
LABEL_606:
...
LABEL_607:
    ...
    goto LABEL_700;
LABEL_608:
    msg = "DLSOIBT- MF (=I1) illegal.    ";
    XERRWD(msg, 30, 8, 0, 1, mf, 0, 0, 0.0, 0.0);
    goto LABEL_700;
LABEL_609:
    msg = "DLSOIBT- MB (=I1) or NB (=I2) illegal.  ";
    XERRWD(msg, 40, 9, 0, 2, mb, nb, 0, 0.0, 0.0);
    goto LABEL_700;
LABEL_610:
    msg = "DLSOIBT- NB (=I1) .lt. 4 illegal.       ";
    XERRWD(msg, 40, 10, 0, 1, nb, 0, 0, 0.0, 0.0);
    goto LABEL_700;
LABEL_611:
...
LABEL_613:
    ...
    goto LABEL_700;
LABEL_614:
    msg = "DLSOIBT- TOUT (=R1) behind T (=R2)      ";
    XERRWD(msg, 40, 14, 0, 0, 0, 0, 2, tout, t);
    msg = "      Integration direction is given by H0 (=R1)  ";
    XERRWD(msg, 50, 14, 0, 0, 0, 0, 1, h0, 0.0);
    goto LABEL_700;
LABEL_615:
    msg = "DLSOIBT- HMAX (=R1) .lt. 0.0  ";
    XERRWD(msg, 30, 15, 0, 0, 0, 0, 1, hmax, 0.0);
    goto LABEL_700;
LABEL_616:
    msg = "DLSOIBT- HMIN (=R1) .lt. 0.0  ";
    XERRWD(msg, 30, 16, 0, 0, 0, 0, 1, hmin, 0.0);
    goto LABEL_700;
LABEL_617:
    msg = "DLSOIBT- RWORK length needed, LENRW (=I1), exceeds LRW (=I2)";
    XERRWD(msg, 60, 17, 0, 2, lenrw, lrw, 0, 0.0, 0.0);
    goto LABEL_700;
LABEL_618:
    msg = "DLSOIBT- IWORK length needed, LENIW (=I1), exceeds LIW (=I2)";
    XERRWD(msg, 60, 18, 0, 2, leniw, liw, 0, 0.0, 0.0);
    goto LABEL_700;
LABEL_619:
    msg = "DLSOIBT- RTOL(=I1) is R1 .lt. 0.0       ";
    XERRWD(msg, 40, 19, 0, 1, i, 0, 1, rtoli, 0.0);
    goto LABEL_700;
LABEL_620:
    msg = "DLSOIBT- ATOL(=I1) is R1 .lt. 0.0       ";
    XERRWD(msg, 40, 20, 0, 1, i, 0, 1, atoli, 0.0);
    goto LABEL_700;
LABEL_621:
    ewti = ARRAY1D(rwork, lewt + i - 1);
    ...
    goto LABEL_700;
LABEL_622:
    msg = "DLSOIBT- TOUT(=R1) too close to T(=R2) to start integration.";
    XERRWD(msg, 60, 22, 0, 0, 0, 0, 2, tout, t);
    goto LABEL_700;
LABEL_623:
    msg = "DLSOIBT- ITASK = I1 and TOUT (=R1) behind TCUR - HU (= R2)  ";
    XERRWD(msg, 60, 23, 0, 1, itask, 0, 2, tout, tp);
    goto LABEL_700;
LABEL_624:
    msg = "DLSOIBT- ITASK = 4 or 5 and TCRIT (=R1) behind TCUR (=R2)   ";
    XERRWD(msg, 60, 24, 0, 0, 0, 0, 2, tcrit, tn);
    goto LABEL_700;
LABEL_625:
    msg = "DLSOIBT- ITASK = 4 or 5 and TCRIT (=R1) behind TOUT (=R2)   ";
    XERRWD(msg, 60, 25, 0, 0, 0, 0, 2, tcrit, tout);
    goto LABEL_700;
LABEL_626:
    ...
    goto LABEL_700;
LABEL_627:
    msg = "DLSOIBT- Trouble in DINTDY.  ITASK = I1, TOUT = R1";
    XERRWD(msg, 50, 27, 0, 1, itask, 0, 1, tout, 0.0);
    goto LABEL_700;
//
LABEL_700:
...
C               odepack_cpp_real PRECISION T, Y(*), S(*), PDJ(*)
C               INTEGER IAN(*), JAN(*)
...
C     odepack_cpp_real PRECISION ATOL, RTOL, RW, T, TOUT, Y, YDOTI, R4D, EODSQ, DELX
...
C     odepack_cpp_real PRECISION T, Y, G, R4D, EODSQ
C     DIMENSION G(N), Y(N)
C     COMMON /TEST1/ R4D, EODSQ, NM1
...
C
C     SUBROUTINE RESID (N, T, Y, S, R, IRES)
C     odepack_cpp_real PRECISION T, Y, S, R, R4D, EODSQ
C     DIMENSION Y(N), S(N), R(N)
C     COMMON /TEST1/ R4D, EODSQ, NM1
C     CALL GFUN (N, T, Y, R)
...
C     odepack_cpp_real PRECISION T, Y, S, PDJ, R4D, EODSQ
...
C-----------------------------------------------------------------------
 */
void Odepack::DLSODIS(
    ODEPACK_RESIDUAL res, ODEPACK_ADDA3 adda, ODEPACK_JACOBIAN6 jac, int neq,
    odepack_cpp_real *y, odepack_cpp_real *ydoti, odepack_cpp_real &t, odepack_cpp_real tout, int itol, odepack_cpp_real *rtol,
    odepack_cpp_real *atol, int itask, int &istate, int iopt, odepack_cpp_real *rwork, int lrw,
    int *iwork, int liw, int mf, void *user_data)
{
    int i, i1, i2, ier, igo, iflag, imax, imul, imxer, ipflag,
        ipgo, irem, ires, j, kgo, lenyht, leniw, lenrw,
...
LABEL_25:
    ...
    meth = mf1 / 10;
    miter = mf1 - 10 * meth;
    if (moss < 0 || moss > 4) goto LABEL_608;
    if (miter == 2 && moss == 1) moss++;
    if (miter == 2 && moss == 3) moss++;
    if (miter == 1 && moss == 2) moss--;
    if (miter == 1 && moss == 4) moss--;
    if (meth < 1 || meth > 2) goto LABEL_608;
    if (miter < 1 || miter > 2) goto LABEL_608;
// Next process and check the optional inputs. --------------------------
    if (iopt == 1) goto LABEL_40;
...
//-----------------------------------------------------------------------
    ...
    ARRAY1D(iwork, 17) = lenrw;
    leniw = 30;
    if (moss != 1 && moss != 2) leniw = leniw + n + 1;
    ARRAY1D(iwork, 18) = leniw;
    if (lenrw > lrw) goto LABEL_617;
    if (leniw > liw) goto LABEL_618;
    lia = 31;
    if (moss != 1 && moss != 2) leniw = leniw + ARRAY1D(iwork, lia + n) - 1;
    ARRAY1D(iwork, 18) = leniw;
    if (leniw > liw) goto LABEL_618;
    lja = lia + n + 1;
    lia = std::min(lia, liw);
    ...
    ARRAY1D(iwork, 18) = leniw;
    if (leniw > liw) goto LABEL_618;
    if (moss == 0) leniw = leniw + ARRAY1D(iwork, lic + n) - 1;
    ARRAY1D(iwork, 18) = leniw;
    if (leniw > liw) goto LABEL_618;
    ljc = lic + n + 1;
    lic = std::min(lic, liw);
    ...
    imul = 2;
    if (istate == 3) imul = moss;
    if (istate == 3 && moss == 3) imul = 1;
    if (moss == 2 || moss == 4) imul = 3;
    lrtem = lenyht + imul * n;
    lwtem = lrw - 20 - lrtem;
    lenwk = lwtem;
    lyhn = lwm + lwtem;
...
LABEL_80:
    lyh = lyhn;
    ARRAY1D(iwork, 22) = lyh;
    if (moss != 2 && moss != 4) goto LABEL_85;
...
// DIPREPI and DPREPI do sparse matrix preprocessing. -------------------
    ...
    lewt = std::min(lewt, lrw);
    lacor = std::min(lacor, lrw);
    DIPREPI(neq, y, ydoti, rwork, &ARRAY1D(iwork, lia), &ARRAY1D(iwork, lja),
        &ARRAY1D(iwork, lic), &ARRAY1D(iwork, ljc), ipflag, res, jac, adda, user_data);
    lenrw = lwm - 1 + lenwk + lrest;
    ARRAY1D(iwork, 17) = lenrw;
    if (ipflag != -1) ARRAY1D(iwork, 23) = ipian;
    if (ipflag != -1) ARRAY1D(iwork, 24) = ipjan;
...
// Set flag to signal changes to DSTODI.---------------------------------
    jstart = -1;
    if (nq <= maxord) goto LABEL_94;
// MAXORD was reduced below NQ.  Copy YH(*,MAXORD+2) into YDOTI. --------
    for (i = 1; i <= n; ++i) {
        ARRAY1D(ydoti, i) = ARRAY1D(rwork, i + lsavf - 1);
    }
LABEL_94:
    if (n == nyh) goto LABEL_200;
// NEQ was reduced.  Zero part of YH to avoid undefined references. -----
...
// Compute initial dy/dt, if necessary, and load it into YH.-------------
    lyd0 = lyh + n;
    if (istate != 0) goto LABEL_120;
    DAINVGS(neq, t, y, &ARRAY1D(rwork, lwm), &ARRAY1D(rwork, lwm), &ARRAY1D(rwork, lacor),
        &ARRAY1D(rwork, lyd0), ier, res, adda, user_data);
    nfe++;
    igo = ier + 1;
    if (igo == 1) {
        goto LABEL_120;
...
// Load H with H0 and scale YH(*,2) by H0. ------------------------------
    h = h0;
    for (i = 1; i <= n; ++i) {
        ARRAY1D(rwork, i + lyd0 - 1) = h0 * ARRAY1D(rwork, i + lyd0 - 1);
    }
    goto LABEL_270;
//-----------------------------------------------------------------------
// Block D.
...
//-----------------------------------------------------------------------
    DSTODI(neq, y, &ARRAY1D(rwork, lyh), nyh, &ARRAY1D(rwork, lyh), &ARRAY1D(rwork, lewt),
        ydoti, &ARRAY1D(rwork, lsavf), &ARRAY1D(rwork, lacor), &ARRAY1D(rwork, lwm),
        &ARRAY1D(rwork, lwm), res, adda, jac, &Odepack::DPRJIS, &Odepack::DSOLSS, user_data);
    kgo = 1 - kflag;
    if (kgo == 1) {
        goto LABEL_300;
    } else if (kgo == 2) {
...
LABEL_550:
    ...
    XERRWD(msg, 50, 206, 0, 0, 0, 0, 0, 0.0, 0.0);
    msg = "    error IRES = 3 repeatedly.";
    XERRWD(msg, 30, 206, 1, 0, 0, 0, 0, tn, 0.0);
    istate = -7;
    goto LABEL_590;
...
LABEL_634:
    ...
    XERRWD(msg, 60, 34, 0, 0, 0, 0, 0, 0.0, 0.0);
    ier = -ipflag - 5;
    msg = "     returned error IRES (=I1)";
    XERRWD(msg, 30, 34, 0, 1, ier, 0, 1, tn, 0.0);
//
LABEL_700:
    istate = -3;
    return;
